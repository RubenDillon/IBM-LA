...
1) Dependiendo de la instalacion que hemos hecho para el Front End debemos elegir distintos pasos en adelante. Para poder hacer uso de APIC en nuestra aplicacion si escogimos mantener httpd, debemos modificar el proxy que hemos definido en la VM1 (la que publica la Aplicacion). Para ello debemos editar el archivo /etc/httpd/conf/httpd.conf y cambiar donde dice
...
en el caso de usar Spring Boot, que tiene su propia arquitectura de Apache. Debemos modificar el archivo /etc/httpd/conf.d/api_proxy.conf con la misma idea
...
<VirtualHost *:8080>
    ...
    ErrorLog  /var/log/httpd/api_proxy_error.log
    CustomLog /var/log/httpd/api_proxy_access.log combined

</VirtualHost>


```
 Tambien se sugiere modificar ServerName y poner la direccion IP privada del servidor en /etc/httpd/conf/httpd.conf

```
ServerName 10.241.64.69
```
...
3) Reiniciar el servicio httpd (y en el caso de Spring Boot ... ademas y primero, se debe volver a cargar el demonio)
```
            systemctl daemon-reload
            systemctl restart httpd
```

Para el caso de Spring boot podemos realizar las siguientes validaciones

```
curl -I http://localhost:8080/clienteApp/index.html
```
...
```
curl -i http://localhost:8080/api_proxy/clientes.php?id=1
```
...
4) Si hemos conectado Instana con la aplicacion, podriamos ver la perspectiva de App cuando tenga el suficiente trafico
